      ...
      {step === 1 && (
        <>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="fullName" className="block text-md font-medium text-slate-300 mb-2">
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-white"
                    required
                  />
                  <p className="text-xs text-slate-400 mt-1">आपका असली नाम लोगों को नहीं दिखाया जाएगा।</p>
                </div>
                <div>
                  <label htmlFor="displayName" className="block text-md font-medium text-slate-300 mb-2">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="phone" className="block text-md font-medium text-slate-300 mb-2">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400 mt-1">
                      ...
                      <span>आपका फ़ोन किसी के साथ साझा नहीं किया जाएगा।</span>
                  </div>
                </div>
                <div>
                  <label htmlFor="profession" className="block text-md font-medium text-slate-300 mb-2">
        ...
        <>
            ...
            <div className="relative" ref={languageDropdownRef}>
                ...
                <button
                    ...
                    className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-left flex justify-between items-center"
                    aria-haspopup="listbox"
                    aria-expanded={languageDropdownOpen}
                >
                    <span className={`truncate ${formData.languages.length > 0 ? 'text-slate-100' : 'text-slate-500'}`}>
                ...
                {languageDropdownOpen && (
                    <div className="absolute z-10 mt-1 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-lg max-h-60 overflow-y-auto" role="listbox">
                        <div className="p-2 space-y-1">
                            {LANGUAGES.map(lang => (
                                <label key={lang.value} htmlFor={`lang-${lang.value}`} className="flex items-center p-2 rounded-md hover:bg-slate-700 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        id={`lang-${lang.value}`}
                                        name="languages"
                                        ...
                                        className="h-4 w-4 rounded border-slate-600 text-cyan-600 focus:ring-cyan-500 bg-slate-800"
                                    />
                                    <span className="ml-3 text-sm text-slate-300 select-none">
                                        {lang.label}
                                    </span>
                                </label>
                            ))}
                ...
                <p className="text-xs text-slate-400 mt-1">कम से कम एक भाषा चुनें।</p>
            </div>


            {/* Payment Details */}
            <fieldset className="border border-slate-700 rounded-lg p-4">
              <legend className="px-2 font-semibold text-slate-300">भुगतान के लिए बैंक/UPI विवरण <span className="text-red-500">*</span></legend>
              <div className="space-y-4 mt-2">
                  <input type="text" name="bankAccount" value={formData.bankAccount} onChange={handleChange} placeholder="बैंक खाता संख्या" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-white"/>
                  <div className="grid grid-cols-2 gap-4">
                      <input type="text" name="ifsc" value={formData.ifsc} onChange={handleChange} placeholder="IFSC कोड" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-white"/>
                      <input type="text" name="bankName" value={formData.bankName} onChange={handleChange} placeholder="बैंक का नाम" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-white"/>
                  </div>
                  <div className="flex items-center">
                  ...
                  <input type="text" name="upiId" value={formData.upiId} onChange={handleChange} placeholder="UPI ID" className="w-full p-3 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-cyan-500 transition text-white"/>
              </div>
            </fieldset>
        </>
      )}
      
      {error && <p className="text-red-400 bg-red-900/50 p-3 rounded-lg text-center font-medium">{error}</p>}
